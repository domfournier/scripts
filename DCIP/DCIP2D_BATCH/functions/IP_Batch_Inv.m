function IP_Batch_Inv(work_directory,argin)
...
%         fprintf(fid,'NULL  ! w.dat\n');
        ...
        fprintf(fid,['OBS LOC_X ' ip_obs_file '\n']);
        fprintf(fid,'TOPO DEFAULT\n');
        fprintf(fid,'MESH FILE dcinv2d.msh\n');
        fprintf(fid,'COND FILE dcinv2d.con\n');
        fprintf(fid,'ALPHA VALUE 1.e-3 1.0 1.0\n');
        fprintf(fid,'CHIFACT DEFAULT \n');
        fprintf(fid,'INIT_MOD VALUE 1e-4 \n');
        fprintf(fid,'REF_MOD VALUE 0.0 \n');
        fprintf(fid,'INVMODE CG \n');
        fclose(fid);

        dos ('"C:\Program Files\UBC-GIFv5\bin\ipinv2d" IP.inp')
        cd ..
