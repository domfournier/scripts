...
work_dir    = 'C:\Users\dominiquef.MIRAGEOSCIENCE\Dropbox\FDEM_Maysam';

dsep = '\';

%% Load input file
obsfile = 'EM1DFM_data.obs';
topofile = 'Topo.topo';
...
[tID,ind] = unique(data{9}(:,3));

xyz = [data{9}(ind,1:2) -data{1}(ind,3)];
% data{7} = abs(data{7});
...
for ii = 1:length(tID)
    ...
    elseif lineID(ii)~=lineID(ii+1) && indx < nstn-1
    ...
    % Offset the receiver
    rxLoc = xyz(ii,1:2) + dl*data{1}(ii,1);
    
    rx(ii,:) = [data{3}(ii) rxLoc z(ii) NaN(1,20) data{7}(ii,1) data{8}(ii,1) -data{7}(ii,2) data{8}(ii,2)];
end
