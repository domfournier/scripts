...
def PFSimulator(prism, survey):
    ...
    out = widgets.interactive(
        ...
        irt=widgets.ToggleButtons(options=['total', 'induced', 'remanent']),
        Q=widgets.FloatSlider(
            min=0., max=10, step=1, value=0, continuous_update=False),
        ...
        Profile_azm=widgets.FloatSlider(
            min=-90, max=90, step=5, value=45., continuous_update=False),
        Profile_len=widgets.FloatSlider(
            min=10, max=diag, step=10, value=Ly, continuous_update=False),
        Profile_ctx=widgets.FloatSlider(
            value=ctx, min=xlim[0], max=xlim[1], step=0.1,
            continuous_update=False, color='black'),
        Profile_cty=widgets.FloatSlider(
            value=cty, min=ylim[0], max=ylim[1], step=0.1,
            continuous_update=False, color='black')
    )
...
def PlotFwrSim(prob, susc, comp, irt, Q, rinc, rdec,
    ...
    MagSurvey2D(
        survey, Profile_ctx, Profile_cty, Profile_azm,
...
def ViewMagSurvey2D(survey):
    ...
    out = widgets.interactive(
        ...
        East=widgets.FloatSlider(
            min=xlim[0]*0.95, max=xlim[1]*1.05, step=10, value=cntr[0],
            continuous_update=False),
        North=widgets.FloatSlider(
            min=ylim[0]*0.95, max=ylim[1]*1.05, step=10, value=cntr[1],
            continuous_update=False),
        ...
        Npts=widgets.BoundedFloatText(
            min=10, max=100, step=1, value=20, continuous_update=False),
        Profile=widgets.ToggleButton(
            description='Profile', value=False)
    )
...
def plotMagSurvey2D(survey, a, b, npts, data=None,
    ...
    PF.Magnetics.plot_obs_2D(
        rxLoc, d=data, fig=fig,  ax=ax, vmin=vmin, vmax=vmax, marker=False
    )
...
def ViewPrism(param):
    ...
    def Prism(
        update, dx, dy, dz, x0, y0, elev,
        prism_inc, prism_dec, View_dip, View_azm, View_lim
    ):
    ...
    out = widgets.interactive(
        ...
        dx=widgets.FloatSlider(
            min=1., max=2000., step=1., value=100, continuous_update=False),
        dy=widgets.FloatSlider(
            min=1., max=2000., step=1., value=500, continuous_update=False),
        dz=widgets.FloatSlider(
            min=1., max=2000., step=1., value=500, continuous_update=False),
        x0=widgets.FloatSlider(
            min=cntr[0]-1000, max=cntr[0]+1000, step=1., value=cntr[0],
            continuous_update=False),
        y0=widgets.FloatSlider(
            min=cntr[1]-1000, max=cntr[1]+1000, step=1., value=cntr[1],
            continuous_update=False),
        elev=widgets.FloatSlider(
            min=-1000., max=1000., step=1., value=0., continuous_update=False),
        prism_inc=(-90., 90., 5.),
        prism_dec=(-90., 90., 5.),
